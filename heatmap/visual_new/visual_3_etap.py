...
canvas_width = num_x * scale_x + (num_x + 1) * separation
canvas_height = num_y * scale_y + (num_y + 1) * separation
...
for j in range(num_y):
    for i in range(num_x):
        ...
        # Position on canvas
        x0 = separation + (scale_x + separation) * i
        y0 = separation + (scale_y + separation) * j
        x1 = x0 + scale_x
        y1 = y0 + scale_y
...
plt.tight_layout()
plt.savefig(f"{path_to_save_figure}3_etap.png", dpi=300)
# plt.show()
