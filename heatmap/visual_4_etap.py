...
gap = 0.0
canvas_w = num_cols * img_scale_x + (num_cols + 1) * gap
canvas_h = num_rows * img_scale_y + (num_rows + 1) * gap
...
for j in range(num_rows):
    for i in range(num_cols):
        ...
        x0 = gap + (img_scale_x + gap) * i
        y0 = gap + (img_scale_y + gap) * j
        x1 = x0 + img_scale_x
        y1 = y0 + img_scale_y
...
plt.tight_layout()
plt.savefig(f"{save_dir}4_etap_crop_heatmap.png", dpi=300)
plt.close(fig_crop)
...
plt.tight_layout()
plt.savefig(f"{save_dir}4_etap_weed_heatmap.png", dpi=300)
plt.close(fig_weed)
